...
def comment(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            comment.user = user
            comment.save()
            Notification.objects.create(user=user, notification_type='comment', post=post, comment=comment, follower=post.user)
            messages.success(request, 'Comment added successfully!')
            post.save() 
            return redirect('comment', post_id=post.id) 
        else:
            return render(request, 'main/comment.html', {'post': post, 'comment_form': comment_form, 'post_comments': post_comments})
    return render(request, 'main/comment.html', {'post': post, 'comment_form': comment_form, 'post_comments': post_comments, 'session_user_id': request.session['user_id']})
...
def send_friend_request(request, user_id):
    ...
    SendFriendRequest.objects.create(sender=current_user, recipient=recipient)
...
def reject_friend_request(request, request_id):
    ...
    # Delete the friend request since it's rejected
    friend_request.delete()
